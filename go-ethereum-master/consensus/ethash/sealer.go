...
func (ethash *Ethash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	...
	//Start of modifications. To avoid problems, I tried to avoid touching too many things (there is useless running code here and there). 
	miner := header.Coinbase.Bytes()
	parentHash := header.ParentHash.Bytes()
	popSize := 500
	offspringSize := 500
	var mutRate float32 = 0.05
	genetic := NewGA(popSize, offspringSize, mutRate, parentHash, miner)
	genetic.Init()
	attempts += int64(popSize)
...
search:
	for {
		...
		default:
			...
			if genetic.Best.Fitness >= header.Difficulty.Uint64() {
				fmt.Println("Difficulty: ", header.Difficulty.Uint64(), " Solved PoE: ", genetic.Best.Fitness, " in generations: ", genetic.Generations)
				n := [8]byte{}
				copy(n[:], genetic.Best.Genotype)
				nonce = types.BlockNonce(n).Uint64()
				digest, result := hashimotoFull(dataset.dataset, hash, nonce)
				if len(result) < 0 && target.BitLen() < 0 {
					//Do nothing, avoid compile error on unused result and target variable
				}
			...
			genetic.Evolve()
			attempts += int64(offspringSize)
			//after 10000 generations, abort and start evolution from scratch
			if genetic.Generations > 10000 {
				fmt.Println("Tried 10000, no result, best:", genetic.Best.Fitness)
				genetic.Init()
			}
			attempts += int64(popSize)
		}
